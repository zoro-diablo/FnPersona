...
export const partnerSlice = createSlice({
    ...
    reducers: {
      ...
      addRow: (state, action) => {
        ...
        state.tableData[monthIndex].total = state.tableData[monthIndex].data.reduce((total, item) => total + item.amount, 0);
      },
      editData: (state, action) => {
        ...
        if (itemIndex !== -1) {
          ...
          state.tableData[monthIndex].total = monthData.reduce((total, item) => total + item.amount, 0);
        }
      },
      deleteRow: (state, action) => {
        ...
        state.tableData[monthIndex].total = filteredData.reduce((total, item) => total + item.amount, 0);
      },
    },
  });

export const { editData, deleteRow , addRow } = partnerSlice.actions;
